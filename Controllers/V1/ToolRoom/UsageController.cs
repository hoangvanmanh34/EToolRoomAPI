...
{    
    ...
    public sealed class UsageController : BaseApiController
    {
        ...
        [HttpGet]
        [Authorize(Roles = "System,Dbo.OrderDetail.Get")]
        public IHttpActionResult GetListUsageEquipmentByModel([FromUri] OrderDetailCriteria criteria)
        {
            try
            {
                var result = OrderDetailDao.GetListUsageEquipmentByModel(criteria.EquipmentType, criteria.ModelName, criteria.FromDate, criteria.DueDate);
                return DataOk(result.ToModels());
            }
            catch (Exception e)
            {
        ...
        [HttpGet]
        [Authorize(Roles = "System,Dbo.OrderDetail.Get")]
        public IHttpActionResult GetListUsageConsumablesByModel([FromUri] OrderDetailCriteria criteria)
        {
            try
            {
                var result = OrderDetailDao.GetListUsageConsumablesByModel(criteria.EquipmentType, criteria.ModelName, criteria.FromDate, criteria.DueDate);
                return DataOk(result.ToModels());
            }
            catch (Exception e)
            {
